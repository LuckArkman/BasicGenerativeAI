...
public static class TrainingDataHelper
{
        ...
        int batchSize)
    {
        ...
        Console.WriteLine($"Primeiros 10 tokens: [{string.Join(", ", allTokenIds.Take(10))}]");

        if (!tokenizerService.IsEndOfSequenceToken(allTokenIds.Last()))
        {
            allTokenIds.Add((long)tokenizerService.EOSTokenId);
        }
        ...
        for (int i = 0; i < allTokenIds.Count - 1; i += stepSize)
        {
            ...
            while (inputSlice.Count < maxSequenceLength)
            {
                inputSlice.Add(padTokenId);
            }
            while (targetSlice.Count < maxSequenceLength)
            {
                targetSlice.Add(padTokenId);
            }
        ...
        if (sequences.Count == 0 && allTokenIds.Count > 1)
        {
            var inputSlice = allTokenIds.Take(maxSequenceLength).ToList();
            var targetSlice = allTokenIds.Skip(1).Take(maxSequenceLength).ToList();
